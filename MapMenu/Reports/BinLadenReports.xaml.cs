...
namespace MapMenu.Reports
{
    ...
    /// </summary>
    public partial class BinLadenReports
    {
        private int CMonth;
        private int CYear;

        public BinLadenReports()
        {
            InitializeComponent();
            if (App.IsInEditMode)
            {
        ...
        private void RefreshButtons()
        {
            ...
            var Categories = from category in App.ActiveDatabase.DatabaseEntities.Category
                             where category.Sections.SectionID == Util.BinLadenReports
                             select category;
        ...
        {
            DataUpload de = new DataUpload(Util.ApplicationRoot, true, null, Util.BinLadenReports,
