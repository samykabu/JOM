...
namespace MapMenu.Studies
{
    /// <summary>
    /// Interaction logic for ConstructionDetails.xaml
    /// </summary>
    public partial class UtilitiesDesignStudies
    {
        private int CMonth=1;
        private int CYear=2009;

        public UtilitiesDesignStudies()
        {
            InitializeComponent();
            if (App.IsInEditMode)
            {
        ...
        private void RefreshButtons()
        {
            ...
            var Categories = from category in App.ActiveDatabase.DatabaseEntities.Category
                             where category.Sections.SectionID == Util.UtilitiesDesignBrief
                             select category;
        ...
        {
            DataUpload de = new DataUpload(Util.ApplicationRoot, true, null, Util.UtilitiesDesignBrief,
