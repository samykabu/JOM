...
namespace MapMenu.Reports
{
    ...
    /// </summary>
    public partial class GeneralLocation
    {
        private int CMonth;
        private int CYear;
        ...
        private void RefreshButtons()
        {
            ...
            var Categories = from category in App.ActiveDatabase.DatabaseEntities.Category
                             where category.Sections.SectionID == Util.GeneralLocation
                             select category;
        ...
        private void ShowCategoryDetails(object sender, RoutedEventArgs e)
        {
            ...
            if (records.Count > 0)
            {
                ...
                switch (dtype)
                {
                    ...
                    case DataTypes.Folder: // Video Galary
                        Design3DView View3D = new Design3DView(records[0], false);
                        View3D.ShowDialog();
                        if (View3D.DataChanged)
                            ySelector_DateChanged(CurSelectedDate.Year, CurSelectedDate.Month);
                        break;
        ...
        {
            DataUpload de = new DataUpload(Util.ApplicationRoot, true, null, Util.GeneralLocation,
                                           new DateTime(CYear, CMonth, 1));
            de.DisableDateSelection();
            de.ShowDialog();
            RefreshButtons();
        }
